...
class KalibrasiExport implements FromView, WithHeadings, WithStyles
{
    use Exportable;
    ...
    public function __construct($search, $dateawal, $dateakhir)
    {
        $this->search = $search;
    ...
    public function view(): View
    {
        ...
        if ($this->search) {
            $kalibrasi->where('name', 'like', "%{$this->search}%");
        }

        if ($this->dateawal) {
            $kalibrasi->where('date', '>=', "{$this->dateawal}");
        }

        if ($this->dateakhir) {
            $kalibrasi->where('date', '<=', "{$this->dateakhir}");
        }

        $kalibrasi = $kalibrasi->orderBy('created_at', 'desc')->get();

        return view('exports.kalibrasi', [
            'kalibrasi' => $kalibrasi
        ]);
    }
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        // Isi judul
        $sheet->setCellValue('A1', 'RIWAYAT PENGAJUAN KALIBRASI');
