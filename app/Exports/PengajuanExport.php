...
class PengajuanExport implements FromView, WithHeadings, WithStyles
{
    use Exportable;
    ...
    public function __construct($search, $commodity, $dateawal, $dateakhir)
    {
        $this->search = $search;
    ...
    public function view(): View
    {
        ...
        if ($this->search) {
            $pengajuan->where('name', 'like', "%{$this->search}%");
        }

        if ($this->commodity) {
            $pengajuan->where('type_commodity', $this->commodity);
        }

        if ($this->dateawal) {
            $pengajuan->where('date', '>=', "{$this->dateawal}");
        }

        if ($this->dateakhir) {
            $pengajuan->where('date', '<=', "{$this->dateakhir}");
        }
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        // Isi judul
        $sheet->setCellValue('A1', 'RIWAYAT PENGAJUAN SERTIFIKAT');
